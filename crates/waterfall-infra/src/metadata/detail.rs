...
fn read_video_detail(locator: &str) -> Result<MediaDetail, MetadataReadFailure> {
    let mut file = File::open(locator).map_err(|error| {
        MetadataReadFailure::new(format!("failed to open video detail: {error}"))
    })?;
    let file_len = file
        .metadata()
        .map_err(|error| {
            MetadataReadFailure::new(format!("failed to inspect video detail: {error}"))
        })?
        .len();
...
fn read_audio_detail(locator: &str) -> Result<MediaDetail, MetadataReadFailure> {
        ...
        .map_err(|error| MetadataReadFailure::new(format!("failed to read WAV detail: {error}")))?
    {
        ...
        file.seek(SeekFrom::Start(0))
            .map_err(|error| MetadataReadFailure::new(error.to_string()))?;
        let mut header = Vec::with_capacity(file_len.min(ID3_READ_LIMIT) as usize);
        file.take(ID3_READ_LIMIT)
            .read_to_end(&mut header)
            .map_err(|error| {
                MetadataReadFailure::new(format!("failed to read audio tags: {error}"))
            })?;
        read_id3v2_tags(&header, &mut detail);
        if header.starts_with(b"ID3")
            || header.windows(2).any(|bytes| {
                bytes == [0xff, 0xfb] || bytes == [0xff, 0xf3] || bytes == [0xff, 0xf2]
            })
        {
...
fn read_iso_bmff_detail<R: Read + Seek>(
    reader: &mut R,
    file_len: u64,
) -> io::Result<Option<VideoDetail>> {
    let mut cursor = 0u64;
...
fn read_video_track_codec<R: Read + Seek>(
    reader: &mut R,
    trak: IsoBox,
...
fn read_wav_detail<R: Read + Seek>(
    reader: &mut R,
    file_len: u64,
    ...
    while cursor.saturating_add(8) <= file_len {
        ...
        let size = u64::from(u32::from_le_bytes(
            chunk[4..8].try_into().expect("chunk size"),
        ));
        ...
        if &chunk[..4] == b"fmt " && size >= 12 && payload.saturating_add(12) <= file_len {
            ...
            byte_rate = Some(u32::from_le_bytes(
                fmt[8..12].try_into().expect("byte rate"),
            ));
    ...
    let duration_ms = match (data_size, byte_rate) {
        (Some(data), Some(rate)) if rate > 0 => {
            data.checked_mul(1000).map(|value| value / u64::from(rate))
        }
        _ => None,
    };
...
fn read_id3v2_tags(data: &[u8], detail: &mut AudioDetail) {
    ...
    while cursor.saturating_add(10) <= end {
        ...
        let size = if version == 4 {
            synchsafe_u32(size_bytes).unwrap_or(0)
        } else {
            u32::from_be_bytes(size_bytes.try_into().expect("frame size"))
        } as usize;
...
fn synchsafe_u32(data: &[u8]) -> Option<u32> {
    if data.len() != 4 || data.iter().any(|byte| byte & 0x80 != 0) {
        return None;
    }
    Some(
        (u32::from(data[0]) << 21)
...
mod tests {
    ...
    fn reads_mp4_duration_from_mvhd() {
        ...
        let box_info = read_iso_box(&mut cursor, 0, mvhd.len() as u64)
            .unwrap()
            .unwrap();
        assert_eq!(
            read_mvhd_duration(&mut cursor, box_info).unwrap(),
            Some(12_345)
        );
    }
