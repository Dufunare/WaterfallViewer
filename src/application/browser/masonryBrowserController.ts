...
import type { MasonryLayoutNode } from "../../layout/masonry/masonryLayout";
...
export interface MasonryBrowserSnapshot {
...
export interface MasonryBrowserOptions {
  gap?: number;
...
export interface MasonryBrowserDependencies {
...
export type MasonryBrowserListener = (snapshot: MasonryBrowserSnapshot) => void;
...
export class MasonryBrowserController {
  ...
  readonly #flow: MasonryFlowModel;
  readonly #listeners = new Set<MasonryBrowserListener>();
  ...
  constructor(dependencies: MasonryBrowserDependencies, options: MasonryBrowserOptions = {}) {
    ...
    this.#flow = new MasonryFlowModel({
  ...
  get snapshot(): MasonryBrowserSnapshot {
    return cloneSnapshot(this.#snapshot);
  }

  subscribe(listener: MasonryBrowserListener): () => void {
    this.#assertActive();
  ...
  #refresh(): void {
    ...
    const columnCount = calculateColumnCount(viewport.width, this.#options);
    this.#flow.configure({
      viewport: {
    ...
    for (const node of renderNodes) {
      ...
      this.#ensureThumbnail(session.id, media, node, priority, viewport.devicePixelRatio);
    }
    ...
    this.#snapshot = {
      ...
      columnWidth: this.#flow.columnWidth,
      tiles,
    };
    this.#publish();
  }
  ...
  #assertActive(): void {
    if (this.#disposed) {
      throw new Error("masonry browser controller is disposed");
    }
  }
}

function emptySnapshot(): MasonryBrowserSnapshot {
  return {
    sessionId: null,
...
function resolveOptions(options: MasonryBrowserOptions): ResolvedBrowserOptions {
  ...
  if (!Number.isInteger(resolved.maxThumbnailEdge) || resolved.maxThumbnailEdge <= 0) {
...
function calculateThumbnailEdge(
  ...
  const requested = Math.ceil(Math.max(node.width, node.height) * devicePixelRatio);
...
function cloneSnapshot(snapshot: MasonryBrowserSnapshot): MasonryBrowserSnapshot {
  return {
    ...snapshot,
    scanState:
